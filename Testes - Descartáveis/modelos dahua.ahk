...
SELECT [ip]
 ...
 Loop,% a.Count() {
	 OutputDebug % tipos
	url := "http://admin:tq8hSKWzy5A@" a[A_Index+1,1] "/cgi-bin/magicBox.cgi?action=getDeviceType"
	If regexmatch( tipos, StrRep( http(url,,1), , "type=" ) )
		continue
	tipos .= StrRep( http(url,,1), , "type=" ) "`n"
 }
 MsgBox % Clipboard := tipos