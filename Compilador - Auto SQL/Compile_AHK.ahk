...
[AHK2EXE]
Exe_File=C:\users\dsantos\desktop\executáveis\Compile_AHK.exe
Created_Date=1
[VERSION]
Set_Version_Info=1
...
Inc_File_Version=1
Internal_Name=Kahmpiler
Legal_Copyright=WTFL
Original_Filename=Kahmpiler
Product_Name=Kahmpiler
...
; delete temporary files of last compilation
	OutputDebug % "clean"
	Clean_Directory()		;	ok
	OutputDebug % "constant"
	GoSub, DefineConstants	;	ok
	OutputDebug % "portable"
	GoSub, CheckPortableApp	;	ok
	OutputDebug % "Language"
	GoSub, ReadLanguage		;	ok
	OutputDebug % "Check install"
	GoSub, CheckInstall		;	ok
	OutputDebug % "Command Line"
	GoSub, CommandLine		;	ok
;
...
; INI File
	...
	; If ( s_INI <> ERROR )	{	; Ini Params included in script file
	; 	SaveIniInScript	=	1
	; 	In_File_Content_Split := StrSplit(In_File_Content, "*/ `; CONFIG END")
	; 	In_File_Settings	:= StrReplace( In_File_Content_Split[1] , "/* CONFIG START" )
	; 	In_File_Content		:= In_File_Content_Split[2]
	; 	MsgBox % In_File_Content_Split[2] "`n" In_File_Content_Split[1]
	; 	FileDelete,%	Ini_File
	; 	FileAppend,%	In_File_Settings,%	Ini_File
	; 	_READ_INI( Ini_File )
	; 	SplitPath , Exe_File , s_File_Name
...
; ================================================================================
	; Show Gui
		...
		GuiShow:
			...
			Gui , Tab , 1
				...
				Gui , Font , cBlack
				Gui , Add , Edit , x+20 yp h20 w280 vGui_Exe_File , C:\users\dsantos\desktop\executáveis\%s_File_Name%
				...
				; Gui , Add , Checkbox , xs y+17 w80 Checked%No_UPX% vGui_No_UPX , %Lang_No_UPX%
				Gui , Add , Checkbox , x+1 yp w120 Checked%No_Parameters% vGui_No_Parameters gSet_Parameters, %Lang_No_Parameters%
				...
				; Gui , Add , Checkbox , x+20 yp w80 Checked%Show_Pwd% vGui_Show_Pwd gGuiShowPwd , %Lang_Show_Pwd%_Parameters%

				; Gui , Font , cBlack
				; if ( Gui_No_Parameters = 0 )
				; 	Gui , Add , Checkbox , x+1 yp h20 w200 Checked1 vGui_SQL , Salvar no Banco de Dados %Lang_SQL%
				; Else
				; 	Gui , Add , Checkbox , x+1 yp h20 w200 Checked0 vGui_SQL , Salvar no Banco de Dados %Lang_SQL%
				...
				Gui , Font , cBlack
				if ( !No_Parameters )
					Gui , Add , Edit , x+20 yp h20 w280 vGui_Run_After ,%	Set_Parameters_Option()
				Else
	...
	; Gui Set Parameter
		...
		Gui , Submit , NoHide
			if ( Gui_No_Parameters = 0 )
				GuiControl, , Gui_Run_After ,% Set_Parameters_Option( Gui_Run_After )
			Else
				GuiControl, , Gui_Run_After , %Run_After%
		Return
	;
	; Change Button Names
			...
			Loop % a_%Type%0	{
				...
				If(increment) {
					...
					Else	{
						...
						; OutputDebug Gui_T%Type%%ReverseIndex% %value%
						increment = 0
					}
				}
				Else
...
; URL Function
	...
	_READ_INI(s_Ini_File)	{
		Global
		Local s_INI
		; MsgBox % s_Ini_File "`n" Defaults_Ini "`n" AHK_Section "`n" Exe_File
		...
		IniRead , s_INI , %s_Ini_File% , %RES_Section% , File_Obs , ERROR
			If (s_INI = "ERROR")
				Loop , parse , In_File_Content , `n
					If ( InStr( A_LoopField , "File_Obs =" ) > 0 )
						File_Obs := StrReplace( A_LoopField , "File_Obs =" )
...
; Write INI file
	_WRITE_INI(s_Ini_File)	{
		...
		Loop , %Icon_Count% {
			...
			if ( A_Index = 1 ) ; Main Icon
				Main_Icon := ";@Ahk2Exe-SetMainIcon " TempIcon
			;

			; MsgBox % TempIconName "`t" TempIcon
...
; Save Ini Settings in Script File
	...
	_SaveIniInScript()	{
		...
		In_File_Content := "/* CONFIG START" . new_line
			. Ini_File_Content
			. new_line . "*/ `; CONFIG END" . new_line . Main_Icon . new_line
			. In_File_Content
...
;	Compile_AHK
	...
	_Compile_AHK()	{	;	COMPILADOR
		Global
		Local s_IN , s_OUT , s_PASS , s_NoDecompile , s_CMD , s_Log , Old_Compression , s_ICON
		...
		s_ICON := StrLen( Icon_1 ) > 0 ? " /icon """ Icon_1 """" : ""
...
;	Run after
	...
	_Run_After()	{
		Global
		Local s_CMD , s_ERR , s_Log , r_CMD 
		Error_Message := "Run after Error!`n"

		r_CMD := Run_After
		ToolTip, Executing Run After Command:`n%r_CMD%
		Run, %r_CMD%
		ToolTip
		Return
	}
...
;	Create a ToolTip on Mouseover
	...
	WM_MOUSEMOVE()	{
		...
		DisplayToolTip:
			; OutputDebug % CurrControl "`t" Gui_ "`t" Lang_
...
;	Define as constantes and variáveis
...
;	Read language file
	...
	ReadLanguage:
		...
		If( !Portable_App )	{
			...
			if StrLen( FileExist(Language_Ini) != 0 ) {
...
; ERROR EXIT
	...
	_Error_Exit(sMSG)	{
		ExitApp ; sem mensagem
		global SaveIniInScript, Ini_File
		if ( sMSG = "Compilation canceled!" )
			_EXIT(1)
