...
Interface:
	...
	Gui, individual:Add, Button,%		"x" A_ScreenWidth-115 "	y5								h25						g_informacoes"								,	Informações
	Gui, individual:Add,	Tab3,x5	y5, Relatório Individual|Registros
	nm_usuario_ad := Windows.Users( @usuario )
	Gui, individual:Add, MonthCal,%		"								w230							v@date			g_b_data		Section"
	Gui, individual:Add, Text,%			"								w" A_ScreenWidth-275 "	h25										+Center	0x1000"				,	Buscar
	Gui, individual:Add, Edit,%			"								w" A_ScreenWidth-275 "	h25		v@b_relatorios	g_busca"
	; Gui, individual:Add, Edit,%			"								w" A_ScreenWidth-45 "	h260	v@n_relatorio								+WantTab"
	Gui, individual:Add, Edit,%			"								w" A_ScreenWidth-715 "	h260	v@e_relatorios	g_e_relatorio	+ReadOnly	+WantTab"
		gui.Font( "individual:" )
	Gui, individual:Add, ListView,%		"								w" A_ScreenWidth-275 "	R12	v@lv AltSubmit	g_s_relatorio	Grid"						,	Data|Relatório|Nome|id|pre|edicoes|ip
		gui.Font( "individual:", "S10" )
	Gui, individual:Add, Button,%		"x" A_ScreenWidth-225 "	y625	w200					h30						g_i_relatorio"								,	INSERIR
		Gui, individual:Add, GroupBox,%	"x10					y0		w" A_ScreenWidth-25 "	h620	vb_geral"
		Gui, individual:Add, GroupBox,%	"x10					y330	w" A_ScreenWidth-25 "	h290	vb_inserir"
		Gui, individual:Add, GroupBox,%	"x" A_ScreenWidth-235 "	y612	w220					h50		vb_botoes"
		Gosub, Carrega_Relatorios
	Gui, individual:Show,%				"x-2					y0		w" A_ScreenWidth																			,	Relatório Individual
	OutputDebug % "Show"
return

Carrega_Relatorios:
	Gui,	individual:Default
	Gui,	individual:ListView, @lv
	relatorios:={}
	select =
		(
		SELECT	[data]
...
_busca:
	...
	LV_Delete()
	if (@date != SubStr( A_Now, 1, 8 )
	&&	StrLen(@b_relatorios) = 0 )			{
