;	Início registro
	NewRegister()				{
		url	:=	token "/sendMessage?text=" html_encode("Bem vindo ao Monitoramento - Cotrijal`n`nEnvie seu código para iniciar o registro.") "&chat_id=" from_id
...
;	Código inválido
	WrongCode()					{
		if(fail[MAP(fail,from_id)].falhas="")	{
			falhou=1
			restam=2
			fail.Push({falhas:falhou,id:from_id,resta:restam})
			OutputDebug % fail[MAP(fail,from_id)].falhas " entrou"
			u:="INSERT INTO [telegram].[dbo].[users] ([name],[fails],[chat_id]) VALUES ('falha_" from_id "','1','" from_id "')"
			}
		Else if(fail[MAP(fail,from_id)].falhas<3){
			restam:=fail[MAP(fail,from_id)].resta-1
			falhou:=fail[MAP(fail,from_id)].falhas+1
			fail.pop(MAP(fail,from_id))
			fail.Push({falhas:falhou,id:from_id,resta:restam})
			OutputDebug % fail[MAP(fail,from_id)].falhas " meio"
		...
		Else if(fail[MAP(fail,from_id)].falhas=3){
			...
			fail.pop(MAP(fail,from_id))
			fail.Push({falhas:falhou,id:from_id,resta:restam})
			OutputDebug % fail[MAP(fail,from_id)].falhas " saiu"
			TooManyFails()
			Return
			}
		Else										{
			OutputDebug % fail[MAP(fail,from_id)].falhas " else"
			TooManyFails()
			Return
			}
...
;	Verifica o código(se ele existir na base de dados) enviando 4 matrículas falsas e 1 real
	CheckCode( mtext, index )	{
		...
		s	:=	sql( u )
		if( debug = "sql" )
			OutputDebug	%	"sql`n`n"	adosql_lq "`n" adosql_le	"`n_____________________________"
