...
Created_Date=1
Run_After="C:\Users\dsantos\Desktop\Executáveis\AHK2BD.exe "agenda_user" "0.0.0.15" """
[VERSION]
Set_Version_Info=1
...
;	GUI 
	...
	;	Tab	1	-	Agenda
		...
		;	FILTROS
			...
			Gui,	Add, DropDownList,	x370	y35						vOperador	g_date			AltSubmit			,	Todos|Operador 1|Operador 2|Operador 3|Operador 4|Operador 5|Operador 6
	...
	;	Tab 5	-	Registros
		...
		;	Modo Admin
			...
			Gui,	Add,	DropDownList,	x245	y125	w1000			vd2		gregistros		%d%									,	|Banheiro|Intervalo
				Gui,	Font
			Gui,	Add,	ListView,		x10		y210	w1235			vlv5					%d%	AltSubmit	Grid	R16			,	Operador|Saída|Retorno|Ausente|Motivo|pkid
	...
	Menu, menudecontexto, Add, Finalizar, end_
	Menu, menudecontexto, Color, 9BACC0
...
GuiContextMenu:	;{	Exibe o menu de finalizar banheiro
	if ( A_GuiControl != "lv5" )
		return
	LV_GetText(pkid_lv5, A_EventInfo, 6 )
	LV_GetText(retorno, A_EventInfo, 3 )
	if (StrLen( pkid_lv5 ) = 0
	||	StrLen( retorno ) > 0 )
		Return
	LV_GetText( operador_lv5 , A_EventInfo , 1 )
...
_date:
	...
	if ( periodo = 1 )	{
		...
		;	GuiControl's
			...
			; else if ( operador = 8 )	{	;	NUNCA será 8
				; op	:=	operador -1
		...
		;	Filtro por data definida
			if ( periodox = 0 )	{
				periodo	:=	"AND DATEDIFF(DAY,a.[data_alerta],'"	mcall	"') = "	periodox
				if ( operador = 1 )
...
carrega_lv:
	...
	sqlv =
		...
		WHERE
			...
			%periodo%
			%_busca%
			AND b.[operador] < '7'	--ignora ocomon, avisos e frota
		ORDER BY
			2
		DESC
		)
...
OnTabSelect:
	...
	if ( tab = 5 )	{	;	registros
		...
		u := sql( u )
		outputdebug % usuarioatual := u[2,1]
		if ( StrLen( u[2,1] ) < 3 )
...
_banheiro:
	if ( SubStr(A_Now,9)<"070000" OR SubStr(A_Now,9) > "200000" )	{
		hoje := SubStr( Datetime( 1 , A_Now ), 1, 10 )
		horav = 20:00:00
	}
	else	{
		hoje := SubStr( Datetime( 1 ,A_Now ), 1, 10 )
...
verifica:	;{	Verifica ao reiniciar o software os estados de banheiro e intervalo
	...
	if ( StrLen(h_null[2,4]) = 0 && h_null[2,6] = "Intervalo" )	{
		...
		GuiControl,	,	_snack,	%botao%
		GuiControl,	Disable,	_bath
		GuiControl,	,	_l_sai,	%	"Saída: " Datetime( 1 , h_null[2,3] )
		GuiControl,	,	_l_volta
		GuiControl,	,	_b_sai
		GuiControl,	,	_b_volta
		snack			=	1
		}
	if ( StrLen( h_null[ 2 , 4 ] ) = 0
	&& h_null[ 2 , 6 ] = "Banheiro" )	{
		botao 			=	Registrar Retorno do Banheiro
		GuiControl,	,	_bath,	%botao%
		GuiControl,	Disable,	_snack
		GuiControl,	,	_l_sai
		GuiControl,	,	_l_volta
		GuiControl,	,	_b_sai,	%	"Saída: " Datetime( 1 , h_null[2,3] )
		GuiControl,	,	_b_volta
		bath			=	1
		}
return
